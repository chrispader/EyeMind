...
experimentConfig = {
	...
	"rooms": ["S","D","K"], #eye-tracking rooms where parrallel sessions will occur
	"participantsPerRoom": 50, #number of participant sessions to be created in each room folder
	"ParticipantsIdPrefix": "P", #prefix used in participant id
	"participantsIdSeed":1, #seed from which to start when assigning ids to participants
	"shuffleStartTaskIndex":1, #shuffle tasks list starting from startIndex=1
	"questionsPerTask": 5, #number of questions per task. Note that the questions within the tasks are not randomized. Only the tasks are randomized

	"template" :  '../../Experiments-data/bpm-ambiguity-2023/template.json', #template file path
	"outDirectory": "../../Experiments-data/bpm-ambiguity-2023/Participant-data/", #output directory
	"linkingSubProcessesModes": ["withinTab"] # ["no","newTab","withinTab"], linking modes accronyms for no-support, symoblic links, breadcrumb respectively
...
class Tests(unittest.TestCase):
	...
	def test_shuflling(self):
		...
		inFilePath =  'test-data/test-session3.json'
