...
async function setupTracking(xScreenDim, yScreenDim) {
        ...
        const setupData = { "action": 'setup', "xScreenDim": xScreenDim, "yScreenDim": yScreenDim};
        const communication = {
...
async function sendSnapshotID(snapshot) {
    ...
    const snapshotData = { "action": 'addSnapshot', "timestamp": snapshot.timestamp, "id": snapshot.id };
    const communication = {
        method: globalParameters.COMMUNICATION_METHOD_TO_ET_SERVER,
        uri: globalParameters.COMMUNICATION_URI_TO_ET_SERVER,
        body: snapshotData,
...
async function sendSimulationEvent(simulationEventTimestamp,fileName,simulationID,simulationEvent) {
        ...
        // send question event data to the eye-tracking server
        const data = {"action": 'addSimulationEvent',
          "simulationEventTimestamp": simulationEventTimestamp,
          "fileName": fileName,
        ...
        try {
            ...
            const msg = 'A problem occured when sending the simulation event event. '+error;
            res.msg = msg
            res.sucess = false;
        }
...
async function processGazeData(clientState,externalProgressWindow,mainWindow) {
    ...
    mainWindow.webContents.send('updateProcessingMessage',"Preparing eye-tracking data",externalProgressWindow);
...
async function requestGazeData(clientState,externalProgressWindow,mainWindow) {
...
async function dataMapped(dataMapped, start,gazeDataSize, externalProgressWindow, mainWindow) {

    //console.log("dataMapped",arguments);

    /// extend fullGazeData with dataMapped
    const state = getState();
    state.processedGazeData.gazeData.push.apply(state.processedGazeData.gazeData,dataMapped);  // check if the use of a global variable here is ok
...
exports.sendSnapshotID = sendSnapshotID;
exports.sendFullSnapshot = sendFullSnapshot;
exports.sendQuestionEvent = sendQuestionEvent;
exports.processGazeData = processGazeData;
exports.dataMapped = dataMapped;
exports.sendClickEvent = sendClickEvent;
exports.sendSimulationEvent = sendSimulationEvent;